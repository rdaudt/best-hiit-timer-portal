...
export default async function handler(req: NodeReq, res: NodeRes) {
  try {
    ...
    const message = error instanceof Error ? error.message : 'Unknown analytics summary failure.';
    res.status(500).json(errorResponse('ANALYTICS_SUMMARY_FAILED', message));
  }
}
